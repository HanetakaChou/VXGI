...
#define CHECK_ERROR(expr, msg) \
    if (!(expr))               \
    this->signalError(__FILE__, __LINE__, msg)

#pragma warning(disable : 4127) // conditional expression is constant

extern ID3D11ComputeShader *g_pMyConeTracingCS = NULL;
extern NVRHI::TextureHandle g_clipmap_opacity_texture = NULL;
extern NVRHI::TextureHandle g_clipmap_illumination_texture = NULL;
extern uint32_t g_MyWidth = 0U;
extern uint32_t g_MyHeight = 0U;
...
namespace NVRHI
{
    ...
    public:
        ...
        PerformanceQuery(RendererInterfaceD3D11 *_parent)
            : parent(_parent), refCount(1), state(NEW), time(0.f)
        {
        }
        ...
        ULONG Release() override
        {
            ULONG result = --refCount;
            if (result == 0)
                parent->destroyPerformanceQuery(this);
            return result;
        }
    };
    ...
    public:
        ...
        ULONG Release() override
        {
            ULONG result = --refCount;
            if (result == 0)
                parent->destroyConstantBuffer(this);
            return result;
        }
    };
    ...
    public:
        ...
        ULONG Release() override
        {
            ULONG result = --refCount;
            if (result == 0)
                parent->destroyInputLayout(this);
            return result;
        }
    };
    ...
    public:
        ...
        __forceinline void AddBytesSSE42(void *p)
        {
    ...
    void SetDebugName(ID3D11DeviceChild *, const char *)
    {
        // TODO:
    }
    ...
    RendererInterfaceD3D11::RendererInterfaceD3D11(IErrorCallback *errorCB, ID3D11DeviceContext *context)
        : context(context), errorCB(errorCB), nvapiIsInitalized(false), insideRenderingPass(false)
    {
    ...
    ShaderHandle RendererInterfaceD3D11::createShader(const ShaderDesc &d, const void *binary, const size_t binarySize)
    {
        ...
        case ShaderType::SHADER_VERTEX:
        {
            ...
            {
                CHECK_ERROR(false, "Without NVAPI support");
            }
...
#define D3D11_FILTER_REDUCTION_TYPE_STANDARD 0
    ...
    void RendererInterfaceD3D11::dispatch(const DispatchState &state, uint32_t groupsX, uint32_t groupsY, uint32_t groupsZ)
    {
        ...
        if ((g_MyWidth == (8U * groupsX)) && (g_MyHeight == (8U * groupsY)) && (1U == groupsZ))
        {
            ID3D11ShaderResourceView *opacity_srv = getSRVForTexture(g_clipmap_opacity_texture, DXGI_FORMAT_R32_FLOAT, ~0U);
            context->CSSetShaderResources(14U, 1U, &opacity_srv);

            ID3D11ShaderResourceView *illumination_srv = getSRVForTexture(g_clipmap_illumination_texture, DXGI_FORMAT_R32_FLOAT, ~0U);
            context->CSSetShaderResources(15U, 1U, &illumination_srv);

            context->CSSetShader(g_pMyConeTracingCS, NULL, 0U);
            context->Dispatch(g_MyWidth, g_MyHeight, 1U);
        }
        else
        {
    ...
    void RendererInterfaceD3D11::debugBeginEvent(char const *name)
    {
        std::u16string u16_name = std::wstring_convert<std::codecvt_utf8_utf16<char16_t>, char16_t>{}.from_bytes(name);
        if (userDefinedAnnotation)
        {
            userDefinedAnnotation->BeginEvent(reinterpret_cast<WCHAR CONST *>(u16_name.c_str()));
        }
    }

    void RendererInterfaceD3D11::debugEndEvent()
    {
        if (userDefinedAnnotation)
        {
    ...
    TextureDesc RendererInterfaceD3D11::getTextureDescFromD3D11Resource(ID3D11Resource *resource, Format::Enum formatOverride)
    {
        ...
        case D3D11_USAGE_DEFAULT:
            returnValue.usage = TextureDesc::USAGE_DEFAULT;
            break;
        case D3D11_USAGE_IMMUTABLE:
            returnValue.usage = TextureDesc::USAGE_IMMUTABLE;
            break;
        case D3D11_USAGE_DYNAMIC:
            returnValue.usage = TextureDesc::USAGE_DYNAMIC;
            break;
        case D3D11_USAGE_STAGING:
            CHECK_ERROR(0, "Staging resources aren't supported");
            break;
        }
    ...
    ID3D11ShaderResourceView *RendererInterfaceD3D11::getSRVForBuffer(BufferHandle _buffer, Format::Enum format)
    {
        ...
        {
            // Trick
            // BC7 -> Byte Address Buffer

            desc11.Format = DXGI_FORMAT_R32_TYPELESS;
            assert(0U == (buffer->desc.byteSize % sizeof(uint32_t)));
            desc11.BufferEx.NumElements = buffer->desc.byteSize / sizeof(uint32_t);
    ...
    void RendererInterfaceD3D11::setModifiedWMode(bool, uint32_t, const float *, const float *)
    {
        CHECK_ERROR(false, "Without NVAPI support");
    }

    void RendererInterfaceD3D11::setSinglePassStereoMode(bool, uint32_t, bool)
    {
        CHECK_ERROR(false, "Without NVAPI support");
    }
    ...
    void RendererInterfaceD3D11::disableSLIResouceSync(ID3D11Resource *)
    {
        // CHECK_ERROR(false, "Without NVAPI support");
    }
    ...
    uint32_t RendererInterfaceD3D11::getAFRGroupOfCurrentFrame(uint32_t)
    {
        CHECK_ERROR(false, "Without NVAPI support");
        return 0;
    }

    void RendererInterfaceD3D11::setEnableUavBarriers(bool, const TextureHandle *, size_t, const BufferHandle *, size_t)
    {
        // CHECK_ERROR(false, "Without NVAPI support");
    }
    ...
    PerformanceQueryHandle RendererInterfaceD3D11::createPerformanceQuery(const char *name)
    {
        ...
        CHECK_ERROR(SUCCEEDED(device->CreateQuery(&descTQ, &query->begin)) &&
                        ...
                        SUCCEEDED(device->CreateQuery(&descTDQ, &query->disjoint)),
