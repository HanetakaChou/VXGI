...
float4 CompositingPS(FullScreenQuadOutput IN) : SV_Target
{
    ...
    // indirect lighting
    {
        float4 indirect_diffuse_and_ambient_occlusion = g_EnableIndirectDiffuse ? t_IndirectDiffuse[IN.position.xy] : 0;
        float3 indirect_specular = g_EnableIndirectSpecular ? t_IndirectSpecular[IN.position.xy].rgb : 0;

        float3 indirect_diffuse = pow(max(0, indirect_diffuse_and_ambient_occlusion.xyz), 0.7);
        float ambient_occlusion = indirect_diffuse_and_ambient_occlusion.w;

        // voxel cone tracing diffuse
        radiance += indirect_diffuse;

        // environment lighting diffuse
        radiance += diffuse_color * g_AmbientColor.rgb * ambient_occlusion;

        // voxel cone tracing specular
        radiance += indirect_specular;
    }
