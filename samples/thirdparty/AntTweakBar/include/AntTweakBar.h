...
#define TW_TYPE_CSSTRING(n) ((TwType)(0x30000000 + ((n)&0xfffffff))) // Null-terminated C Static String of size n (defined as char[n], with n<2^28)
    ...
    TW_API void TW_CALL TwToggleBarFocus(TwBar *bar);
    ...
    // Pass a pointer to this structure as a parameter to TwInit when working with D3D12
    typedef struct CTwD3D12DeviceInfo
    {
        struct ID3D12Device *Device;
        struct ID3D12CommandQueue *DefaultCommandQueue;
        unsigned int /* DXGI_FORMAT */ RenderTargetFormat;
        unsigned int RenderTargetSampleCount;
        unsigned int RenderTargetSampleQuality;
    ...
    TW_API int TW_CALL TwSetD3D12RenderTargetView(void *cpuDescriptorHandle);
